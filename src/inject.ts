  ...
  iframes.forEach((el) => {
    if (el.contentWindow) {
      ...
      element.addEventListener(
        "keydown",
        (e) => captureEvent(e, <Window>el.contentWindow),
        true
      );
    }
  });
...
let range: Range;
let globalEvent: KeyboardEvent;
let lettersRegex: RegExp;

const getPreviousLetterInput = (value: string, carret: number, key: string) => {
...
function getPreviousLetterContent(key: string) {
...
const getInputValueAndCaret = (element: HTMLElement, key: string) => {
  input = <HTMLInputElement>element;
  const elementValue = input.value;
  const carretPosition = input.selectionStart ?? 0;
  if (carretPosition > 0) {
    getPreviousLetterInput(elementValue, carretPosition, key);
  }
};

const createSelectionAndRange = (win: Window, key: string) => {
...
const isEditable = (key: string) =>
  lettersRegex.test(key) && globalEvent.ctrlKey === false;
function captureEvent(e: KeyboardEvent, win: Window) {
  if (isOn) {
...
const createLetterRegex = (letter: string) => {
...
chrome.storage.sync.get(
  ...
  ({ enabled, currSystem }) => {
    ...
    if (typeof currSystem === "undefined") currSystem = "x";
    isOn = enabled;
